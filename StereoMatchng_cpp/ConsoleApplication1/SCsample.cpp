...
SCsample::SCsample(int boardwidth, int boardheight, float squaresize)
{
...
void SCsample::StereoCalibrate(const vector<string>& imagelist, Size boardSize, bool displayCorners, bool useCalibrated, bool showRectified)
{
	//
	// 画像読み込み, チェッカーボード検出
	//
...
bool SCsample::DetectObjectPoints(
...
double SCsample::StereoCalibrate() {

	for (int i = 0; i < 2; i++)
	{
		vector<Mat> *rvecs, *tvecs;
		rvecs = new vector<Mat>; tvecs = new vector<Mat>;
		cv::calibrateCamera(SCsample::ObjectPoints, SCsample::ImagePoints[i], SCsample::ImageSize, SCsample::CameraMatrix[i], SCsample::DistCoeffs[i], *rvecs, *tvecs);
		//delete rvecs, tvecs;
	}
	double rms =
		stereoCalibrate(
			SCsample::ObjectPoints, SCsample::ImagePoints[0], SCsample::ImagePoints[1],
			SCsample::CameraMatrix[0], SCsample::DistCoeffs[0],
			SCsample::CameraMatrix[1], SCsample::DistCoeffs[1],
			SCsample::ImageSize, SCsample::R, SCsample::T, SCsample::E, SCsample::F,
			TermCriteria(TermCriteria::COUNT + TermCriteria::EPS, 100, 1e-5),
			CALIB_FIX_INTRINSIC +
			//CALIB_FIX_PRINCIPAL_POINT +
			//CALIB_FIX_ASPECT_RATIO +
			...
			//CALIB_FIX_K6 +
			0);
	cv::stereoRectify(SCsample::CameraMatrix[0], SCsample::DistCoeffs[0],
		SCsample::CameraMatrix[1], SCsample::DistCoeffs[1],
		SCsample::ImageSize, SCsample::R, SCsample::T,
		SCsample::R1, SCsample::R2, SCsample::P1, SCsample::P2, SCsample::Q,
		CALIB_ZERO_DISPARITY
		+ 0,
		1, SCsample::ImageSize, &SCsample::ValidRoi[0], &SCsample::ValidRoi[1]);

	cv::initUndistortRectifyMap(SCsample::CameraMatrix[0], SCsample::DistCoeffs[0], SCsample::R1, SCsample::P1,
		SCsample::ImageSize, CV_16SC2, SCsample::Rmap[0][0], SCsample::Rmap[0][1]);
	cv::initUndistortRectifyMap(SCsample::CameraMatrix[1], SCsample::DistCoeffs[1], SCsample::R2, SCsample::P2,
		SCsample::ImageSize, CV_16SC2, SCsample::Rmap[1][0], SCsample::Rmap[1][1]);
	return rms;
}

bool SCsample::DetectObjectPointsForStereoCamera2(
	Mat img1, Mat img2,
	Size imgSize,
...
bool SCsample::DetectObjectPointsForStereoCamera(Mat* img)
{
	vector<Point3f> tempOP[2];// = { new vector<Point3f>,new vector<Point3f> };
	vector<Point2f> tempIP[2];// = { new vector<Point2f>,new vector<Point2f> };
	for (int i = 0; i < 2; i++) {
		if (!DetectObjectPoints(img[i], SCsample::ImageSize, SCsample::BoardSize, SCsample::SquareSize, tempOP[i], tempIP[i]))
			return false;
	}

	SCsample::ObjectPoints.push_back((tempOP[0]));
	SCsample::ImagePoints[0].push_back((tempIP[0]));
	SCsample::ImagePoints[1].push_back((tempIP[1]));

	return true;
}

bool SCsample::CalibrateStereoCamera(vector<Mat*> &imgPairList, vector<Mat*>& RimgPairList) {
	SetImageSize(imgPairList[0][0]);
...
int SCsample::StereoRectify(Mat& img1, Mat& img2, Mat& rimg1, Mat& rimg2) {
...
int SCsample::SetImageSize(Mat img) {
	ImageSize = img.size();
	return 1;
}
int SCsample::SetBoardSize(int boardwidth, int boardheight) {
...
bool SCsample::OutputExtrinsicParameter(std::string outputfile) {
	// (7)XMLファイルへの書き出し
...
bool SCsample::InputExtrinsicParameter(std::string inputfile) {
...
bool SCsample::OutputIntrinsicParameter(std::string outputfile, Mat cameraMatrix, Mat distCoeffs) {
	FileStorage fs(outputfile, FileStorage::WRITE);
	if (fs.isOpened()) {
...
bool SCsample::InputIntrinsicParameter(std::string inputfile, Mat& cameraMatrix, Mat& distCoeffs) {
	FileStorage fs(inputfile, FileStorage::READ);
	if (fs.isOpened())
	{
...
bool SCsample::OutputRectifyParameter(std::string outputfile, Mat rmap0, Mat rmap1) {
	FileStorage fs(outputfile, FileStorage::WRITE);
	if (fs.isOpened()) {
